# 什么是分区存储？
事情得从Android 10说起，首次引入了分区存储，类似ios的沙盒机制，应用不可以随意访问外部存储区中的文件。google没有强制性使用，同时给出清单配置属性requestLegacyExternalStorage = true来避免伤及无辜，这样就可以采用原有的存储策略。但在Android 11中，而同时你的应用targetSdkVersion = 30的话，就会忽略此配置属性，强制使用分区存储。
...
一般来说，搞懂属性requestLegacyExternalStorage和preserveLegacyExternalStorage的使用就基本搞定了分区存储的适配。
...
在没有WRITE_EXTERNAL_STORAGE权限的情况下，无论targetSDKVersion是多少，都是报错的。（这里说的情况不包括6.0以下）
